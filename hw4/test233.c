            ...
            for(int y=0;y<im.h;y++){
                for(int xc=0;xc<x;xc++){
                    for(int yc=0;yc<y;yc++){
                        sum+=im.data[xc+yc*im.w+c*im.w*im.h];
                        printf("%f",sum);
                    }
                }
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    image kernel_x=make_image(1,3,1);
    kernel_x.data[0]=-1;
    kernel_x.data[1]=0;
    kernel_x.data[2]=1;
    image kernel_y=make_image(3,1,1);
    kernel_y.data[0]=-1;
    kernel_y.data[1]=0;
    kernel_y.data[2]=1;
    ...
    image Ix=convolve_image(im,kernel_x,0);
    image Iy=convolve_image(im,kernel_y,0);
    image Ix_2=convolve_image(Ix,Ix,0);
    image Iy_2=convolve_image(Iy,Iy,0);
    image IxIy=convolve_image(Ix,Iy,0);
    image IxIt=convolve_image(Ix,It,0);
    image IyIt=convolve_image(Iy,It,0);
    ...
    for (int x=0;x<im.w;x++){
        for(int y=0;y<im.h;y++){
            ...
            set_pixel(S,x,y,2,get_pixel(IxIy,x,y,0));
            set_pixel(S,x,y,3,get_pixel(IxIt,x,y,0));
            set_pixel(S,x,y,4,get_pixel(IyIt,x,y,0));
        }
    }
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            matrix result=matrix_mult_matrix(Minvert,V);
            float vx = result.data[0][0];
            float vy = result.data[0][1];
